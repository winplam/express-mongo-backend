...
import mongoDB from '../config/mongoConnectionTesting'
...
describe('Checking the standalone backend', () => {
  ...
  test('GET:/api/users/:id - Trying to fetch a non-existing single user',
    done => {
      supertest(app)
        .get('/api/users/id')
        .then(res => {
  ...
  test('GET:/api/users/:id - Trying to fetch an existing single user', done => {
    supertest(app)
      ...
      .then(res => {
        ...
        expect(res.headers['content-type']).toStrictEqual('application/json; charset=utf-8')
        expect(res.body).toStrictEqual({ 'error': 'UnauthorizedError: No authorization token was found' })
        done()
      })
  })
  test(
  ...
  test('POST:/api/signin - Attempting to signing in - User not found', done => {
    supertest(app)
      ...
      .then(res => {
        expect(res.statusCode).toEqual(401)
        expect(res.headers['content-type']).toStrictEqual('application/json; charset=utf-8')
        expect(res.body).toStrictEqual({ 'error': 'Email and password don\'t match.' })
        done()
      })
  })
  ...
  test('PUT:/api/users/:id - Attempt to update somebody else`s user profile',
    done => {
      supertest(app)
  ...
  test('DELETE:/api/users/:id - Attempting to delete user without token',
    done => {
      supertest(app)
  ...
  test('DELETE:/api/users/:id - Attempting to deleting a non-existing user',
    done => {
      supertest(app)
  ...
  test('DELETE:/api/users/:id - Attempting to delete somebody else`s account',
    done => {
      supertest(app)
